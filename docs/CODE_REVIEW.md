...
Corrections to the original finding: the backend package is com.sq.caa, not the company-derived package name the finding guessed; and the live-call evidence could not be reproduced here (GET /api/customers returns 401 without a token), but the DTO/service/controller chain settles it without a live call. Also, 'Total amount' and 'Last activity' carry `hidden lg:table-cell`, so below 1024px only the always-visible 'Activity' column is visibly dead; at lg+ all three are.
